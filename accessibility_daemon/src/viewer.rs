...
impl OcrViewer {
    ...
    pub fn select_character(&mut self, line_idx: usize, char_idx: usize) {
        ...
        if let Some(result) = self.state.lookup(line_idx, char_idx, &self.db, &self.deinflector) {
            ...
            println!("No dictionary matches found");
    ...
    pub fn view<'a>(&'a self) -> Element<'a, Message> {
        ...
        if self.selected_word.is_some() {
            ...
            let (panel_width, panel_height) =
                self.state.panel_dimensions(root_width, root_height);

            // Use cached lookup results if available, otherwise show empty
            let dictionary_entries = if !self.state.cached_entries.is_empty() {
                self.state.cached_entries.clone()
            } else {
                Vec::new()
            };

            let dictionary_panel = self
                .dictionary_panel(dictionary_entries, panel_width, panel_height)
                .width(Length::Fill);
            ...
            let overlay_stack = Stack::new().push(root).push(panel);
    ...
    fn dictionary_panel<'a>(
        ...
        if entries.is_empty() {
            content = content.push(Text::new("No dictionary entries found.").size(16));
        }
        ...
        for entry in entries {
            ...
            for group in entry.reading_groups {
                ...
                entry_column = entry_column.push(
                    iced::widget::Space::new().height(Pixels(8.0)),
                );
            }
    ...
    fn headword_section<'a>(
        &self,
        group: FormattedReadingGroup,
    ) -> Container<'a, Message> {
    ...
    fn sense_group<'a>(
        &self,
        sense_group: FormattedSenseGroup,
    ) -> Container<'a, Message> {
