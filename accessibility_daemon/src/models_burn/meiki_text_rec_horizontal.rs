...
extern crate alloc;
// Generated from ONNX "assets/meiki.text.rec.v0.960x32.with_logits.onnx" by burn-onnx
use burn::prelude::*;
use burn::nn::Linear;
...
pub struct Submodule2 {
    ...
    conv2d47: Conv2d,
    conv2d48: Conv2d,
    constant379: burn::module::Param<Tensor<3>>,
    linear1: Linear,
    linear2: Linear,
    linear3: Linear,
...
impl Submodule2 {
    ...
    pub fn new(device: &Device) -> Self {
        ...
        let conv2d48 = Conv2dConfig::new([960, 128], [1, 1])
            ...
            .with_bias(true)
            .init(device);
        let constant379: burn::module::Param<Tensor<3>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                3,
            >::zeros([1, 30, 128], (device, burn::tensor::DType::F32)),
            device.clone(),
            false,
            [1, 30, 128].into(),
        );
    ...
    pub fn forward(
        ...
        let add11_out1 = transpose1_out1.clone().add(constant379_out1);
        ...
        let constant350_out1 = 8i64;
        let mul1_out1 = gather1_out1 * constant350_out1;
        let unsqueeze1_out1 = [mul1_out1 as i64];
        let reshape2_out1 = linear1_out1.reshape([30, -1, 16]);
        let transpose4_out1 = reshape2_out1.permute([1, 0, 2]);
        let reshape3_out1 = linear2_out1.reshape([30, -1, 16]);
        ...
        let constant381_out1: [i64; 1] = [16i64];
        ...
        let mul2_out1 = transpose4_out1
            .mul((constant351_out1.clone()).unsqueeze_dims(&[0isize, 1isize]));
        ...
        let reshape6_out1 = linear4_out1.reshape([30, -1, 128]);
        ...
        (sub1_out1, conv2d47_out1, conv2d46_out1, constant347_out1, constant351_out1)
    }
}
#[derive(Module, Debug)]
...
impl Submodule3 {
    ...
    pub fn forward(
        ...
        let reshape7_out1 = transpose9_out1.reshape([-1, 128, 1, 30]);
        ...
        let concat11_out1: [i64; 3usize] = [&slice2_out1[..], &constant347_out1[..]]
            .concat()
            .try_into()
            .unwrap();
        ...
        let concat12_out1: [i64; 3usize] = [&slice3_out1[..], &constant347_out1[..]]
            .concat()
            .try_into()
            .unwrap();
        ...
        let concat13_out1: [i64; 3usize] = [&slice4_out1[..], &constant347_out1[..]]
            .concat()
            .try_into()
            .unwrap();
...
pub struct Submodule4 {
    constant386: burn::module::Param<Tensor<3>>,
    constant387: burn::module::Param<Tensor<3>>,
    linear7: Linear,
    ...
    linear10: Linear,
    linear11: Linear,
    constant361: burn::module::Param<Tensor<1, Int>>,
    linear12: Linear,
    linear13: Linear,
    #[module(skip)]
    device: Device,
...
impl Submodule4 {
    #[allow(unused_variables)]
    pub fn new(device: &Device) -> Self {
        let constant386: burn::module::Param<Tensor<3>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                3,
            >::zeros([1, 630, 4], (device, burn::tensor::DType::F32)),
            device.clone(),
            false,
            [1, 630, 4].into(),
        );
        let constant387: burn::module::Param<Tensor<3>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                3,
            >::zeros([1, 630, 1], (device, burn::tensor::DType::F32)),
            device.clone(),
            false,
            [1, 630, 1].into(),
        );
        ...
        let linear11 = LinearConfig::new(128, 2).with_bias(true).init(device);
        let constant361: burn::module::Param<Tensor<1, Int>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                1,
                Int,
    ...
    pub fn forward(
        ...
        constant353_out1: Tensor<1>,
        constant347_out1: [i64; 1],
    ) -> (Tensor<3>, Tensor<3>, Tensor<3>, Tensor<4>, Tensor<4>, Tensor<4>, [i64; 1]) {
        ...
        let tile1_out1 = {
            let __repeats: alloc::vec::Vec<usize> = concat15_out1
                ...
                .map(|&v| v as usize)
                .collect();
            constant386_out1.repeat(&__repeats)
        };
        let constant387_out1 = self.constant387.val();
        let mul55_out1 = constant387_out1.mul(concat14_out1.clone());
        ...
        let pow3_out1 = sub3_out1
            .clone()
            .powf((constant352_out1).unsqueeze_dims(&[0isize, 1isize]));
        let reducemean6_out1 = { pow3_out1.mean_dim(2usize) };
        let add27_out1 = reducemean6_out1
            .add((constant353_out1).unsqueeze_dims(&[0isize, 1isize]));
        ...
        let (topk1_out1, __topk_indices_raw) = reducemax1_out1.topk_with_indices(48, 1);
        ...
        let gather5_out1 = gatherelements1_out1.take::<1, 3>(2, constant361_out1);
        let add29_out1 = linear11_out1.add(gather5_out1);
        let split_tensors = add29_out1.split_with_sizes([1, 1].into(), 2);
        let [split5_out1, split5_out2] = split_tensors.try_into().unwrap();
        let squeeze1_out1 = split5_out1.squeeze_dims::<2>(&[-1]);
        let squeeze2_out1 = split5_out2.squeeze_dims::<2>(&[-1]);
        let shape9_out1: [i64; 2] = {
            let axes = &squeeze1_out1.clone().dims()[0..2];
        ...
        let constantofshape1_out1 = Tensor::<
            1,
        >::from_data(
                burn::tensor::TensorData::from([0.5f32 as f64]),
                (&self.device, burn::tensor::DType::F32),
            )
            .reshape([1, 1])
            .expand(shape9_out1);
        let shape10_out1: [i64; 2] = {
            let axes = &squeeze2_out1.clone().dims()[0..2];
            let mut output = [0i64; 2];
            for i in 0..2 {
                output[i] = axes[i] as i64;
            }
        ...
        >::from_data(
            ...
            .reshape([1, 1])
            .expand(shape10_out1);
        let unsqueeze6_out1: Tensor<3> = squeeze1_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze7_out1: Tensor<3> = constantofshape1_out1
            .unsqueeze_dims::<3>(&[-1]);
        let unsqueeze8_out1: Tensor<3> = squeeze2_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze9_out1: Tensor<3> = constantofshape2_out1
            .unsqueeze_dims::<3>(&[-1]);
        let concat17_out1 = burn::tensor::Tensor::cat(
            [unsqueeze6_out1, unsqueeze7_out1, unsqueeze8_out1, unsqueeze9_out1].into(),
            2,
        );
        let gather6_out1 = shape7_out1[1] as i64;
        let unsqueeze10_out1 = [gather6_out1 as i64];
        let constant363_out1: [i64; 1] = [8i64];
        let concat18_out1: [i64; 4usize] = [
            ...
            .try_into()
            .unwrap();
        let reshape11_out1 = concat14_out1.reshape(concat18_out1);
        let transpose13_out1 = reshape11_out1.permute([0, 2, 3, 1]);
        let slice5_out1 = transpose13_out1.clone().slice(s![.., .., .., 0..480]);
        let slice6_out1 = transpose13_out1.clone().slice(s![.., .., .., 480..600]);
        let slice7_out1 = transpose13_out1.slice(s![.., .., .., 600..630]);
        let sigmoid49_out1 = burn::tensor::activation::sigmoid(concat17_out1);
...
pub struct Submodule5 {
    ...
    linear18: Linear,
    linear19: Linear,
    constant300: burn::module::Param<Tensor<2>>,
    linear20: Linear,
    #[module(skip)]
    device: Device,
}
impl Submodule5 {
    ...
    pub fn new(device: &Device) -> Self {
        ...
        let constant300: burn::module::Param<Tensor<2>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                2,
    ...
    pub fn forward(
        &self,
        gatherelements2_out1: Tensor<3>,
        clip1_out1: Tensor<3>,
        constant351_out1: Tensor<1>,
        constant352_out1: Tensor<1>,
        constant353_out1: Tensor<1>,
        sigmoid49_out1: Tensor<3>,
        constant356_out1: Tensor<1>,
        ...
        let reshape12_out1 = linear14_out1.reshape([48, -1, 16]);
        let transpose16_out1 = reshape12_out1.permute([1, 0, 2]);
        let reshape13_out1 = linear15_out1.reshape([48, -1, 16]);
        let reshape14_out1 = linear16_out1.reshape([48, -1, 16]);
        let transpose17_out1 = reshape14_out1.permute([1, 0, 2]);
        let mul57_out1 = transpose16_out1
            .mul((constant351_out1).unsqueeze_dims(&[0isize, 1isize]));
        ...
        let reshape16_out1 = linear17_out1.reshape([48, -1, 128]);
        ...
        let reshape17_out1 = linear18_out1.reshape([-1, 48, 8, 18, 2]);
        let linear19_out1 = self.linear19.forward(add34_out1);
        let reshape18_out1 = linear19_out1.reshape([-1, 48, 8, 18]);
        ...
        let mul59_out1 = reshape17_out1
            .mul((constant300_out1.clone()).unsqueeze_dims(&[0isize, 1isize, 2isize]));
        let unsqueeze11_out1: Tensor<5> = sigmoid49_out1.unsqueeze_dims::<5>(&[2, 3]);
        let slice8_out1 = unsqueeze11_out1.clone().slice(s![.., .., .., .., 2..]);
        let mul60_out1 = mul59_out1.mul(slice8_out1);
        let mul61_out1 = mul60_out1
            .mul((constant356_out1).unsqueeze_dims(&[0isize, 1isize, 2isize, 3isize]));
        let slice9_out1 = unsqueeze11_out1.slice(s![.., .., .., .., 0..2]);
        ...
        let gather7_out1 = shape11_out1[0] as i64;
        let gather8_out1 = shape11_out1[1] as i64;
        let gather9_out1 = shape11_out1[2] as i64;
        let mul62_out1 = add35_out1
            .mul((constant352_out1).unsqueeze_dims(&[0isize, 1isize, 2isize, 3isize]));
        let sub5_out1 = mul62_out1
            .sub((constant355_out1).unsqueeze_dims(&[0isize, 1isize, 2isize, 3isize]));
        let transpose21_out1 = sub5_out1.permute([0, 2, 1, 3, 4]);
        let reshape19_out1 = transpose21_out1.reshape([-1, 48, 18, 2]);
        ...
        let reshape23_out1 = transpose22_out1.reshape([-1, 1, 48, 18]);
        let concat22_out1 = burn::tensor::Tensor::cat(
            [gridsample1_out1, gridsample2_out1, gridsample3_out1].into(),
            3,
        );
        let mul64_out1 = concat22_out1.mul(reshape23_out1);
        let reducesum1_out1 = {
            mul64_out1.sum_dim(3usize).squeeze_dims::<3usize>(&[3])
        };
        let mul65_out1 = gather8_out1 * gather9_out1;
        let unsqueeze14_out1 = [gather7_out1 as i64];
        let unsqueeze15_out1 = [mul65_out1 as i64];
        let constant360_out1: [i64; 1] = [48i64];
        let concat23_out1: [i64; 3usize] = [
            ...
            .try_into()
            .unwrap();
        let reshape24_out1 = reducesum1_out1.reshape(concat23_out1);
        ...
        let linear20_out1 = self.linear20.forward(concat24_out1);
        ...
        (add36_out1, constant360_out1, constant300_out1, concat23_out1)
    }
}
#[derive(Module, Debug)]
pub struct Submodule6 {
    ...
    linear27: Linear,
    linear28: Linear,
    constant372: burn::module::Param<Tensor<1>>,
...
impl Submodule6 {
    ...
    pub fn new(device: &Device) -> Self {
        ...
        let constant372: burn::module::Param<Tensor<1>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                1,
    ...
    pub fn forward(
        ...
        let sub8_out1 = (constant355_out1.clone())
        ...
        let gather10_out1 = shape12_out1[0] as i64;
        ...
        let matmul31_out1 = softmax4_out1
            .matmul(constant372_out1.clone().unsqueeze_dims(&[-1isize]))
        ...
        let concat25_out1: [i64; 3usize] = [
            ...
            .try_into()
            .unwrap();
        let reshape26_out1 = matmul31_out1.reshape(concat25_out1);
        let gather11_out1 = {
        ...
        let add43_out1 = (constant395_out1.clone())
        ...
        let div9_out1 = gather13_out1
            .div((constant394_out1.clone()).unsqueeze_dims(&[0isize]));
        let mul70_out1 = add43_out1.mul(div9_out1.clone());
        let sub9_out1 = gather11_out1.clone().sub(mul70_out1);
        let gather14_out1 = {
        ...
        let add44_out1 = (constant395_out1.clone())
            .unsqueeze_dims(&[0isize])
            .add(gather15_out1);
        let gather16_out1 = {
        ...
        let div10_out1 = gather16_out1.div((constant394_out1).unsqueeze_dims(&[0isize]));
        let mul71_out1 = add44_out1.mul(div10_out1.clone());
        let sub10_out1 = gather14_out1.clone().sub(mul71_out1);
        let gather17_out1 = {
            let sliced = reshape26_out1.clone().slice(s![.., .., 2]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let add45_out1 = (constant395_out1.clone())
            .unsqueeze_dims(&[0isize])
            .add(gather17_out1);
        let mul72_out1 = add45_out1.mul(div9_out1.clone());
        let add46_out1 = gather11_out1.clone().add(mul72_out1);
        let gather18_out1 = {
            let sliced = reshape26_out1.slice(s![.., .., 3]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let add47_out1 = (constant395_out1.clone())
            .unsqueeze_dims(&[0isize])
            .add(gather18_out1);
        let mul73_out1 = add47_out1.mul(div10_out1.clone());
        let add48_out1 = gather14_out1.clone().add(mul73_out1);
        let unsqueeze17_out1: Tensor<3> = sub9_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze18_out1: Tensor<3> = sub10_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze19_out1: Tensor<3> = add46_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze20_out1: Tensor<3> = add48_out1.unsqueeze_dims::<3>(&[-1]);
        let concat26_out1 = burn::tensor::Tensor::cat(
            [unsqueeze17_out1, unsqueeze18_out1, unsqueeze19_out1, unsqueeze20_out1]
                .into(),
            2,
        );
        let split_tensors = concat26_out1.split_with_sizes([1, 1, 1, 1].into(), 2);
        let [split7_out1, split7_out2, split7_out3, split7_out4] = split_tensors
            .try_into()
            .unwrap();
        let squeeze3_out1 = split7_out1.squeeze_dims::<2>(&[-1]);
        let squeeze4_out1 = split7_out2.squeeze_dims::<2>(&[-1]);
        let squeeze5_out1 = split7_out3.squeeze_dims::<2>(&[-1]);
        let squeeze6_out1 = split7_out4.squeeze_dims::<2>(&[-1]);
        let add49_out1 = squeeze3_out1.clone().add(squeeze5_out1.clone());
        let div11_out1 = add49_out1
            .div((constant352_out1.clone()).unsqueeze_dims(&[0isize]));
        let add50_out1 = squeeze4_out1.clone().add(squeeze6_out1.clone());
        let div12_out1 = add50_out1
            .div((constant352_out1.clone()).unsqueeze_dims(&[0isize]));
        let sub11_out1 = squeeze5_out1.sub(squeeze3_out1);
        let sub12_out1 = squeeze6_out1.sub(squeeze4_out1);
        let unsqueeze21_out1: Tensor<3> = div11_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze22_out1: Tensor<3> = div12_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze23_out1: Tensor<3> = sub11_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze24_out1: Tensor<3> = sub12_out1.unsqueeze_dims::<3>(&[-1]);
        let concat27_out1 = burn::tensor::Tensor::cat(
            [unsqueeze21_out1, unsqueeze22_out1, unsqueeze23_out1, unsqueeze24_out1]
                .into(),
            2,
        );
        let linear29_out1 = self.linear29.forward(concat27_out1.clone());
        ...
        let reshape27_out1 = linear31_out1.reshape([48, -1, 16]);
        let transpose26_out1 = reshape27_out1.permute([1, 0, 2]);
        let reshape28_out1 = linear32_out1.reshape([48, -1, 16]);
        let reshape29_out1 = linear33_out1.reshape([48, -1, 16]);
        ...
        let reshape31_out1 = linear34_out1.reshape([48, -1, 128]);
        ...
        let reshape32_out1 = linear35_out1.reshape([-1, 48, 8, 18, 2]);
        let linear36_out1 = self.linear36.forward(add55_out1);
        let reshape33_out1 = linear36_out1.reshape([-1, 48, 8, 18]);
        let softmax6_out1 = burn::tensor::activation::softmax(reshape33_out1, 3);
        let mul76_out1 = reshape32_out1
            .mul((constant300_out1).unsqueeze_dims(&[0isize, 1isize, 2isize]));
        let unsqueeze25_out1: Tensor<5> = concat27_out1.unsqueeze_dims::<5>(&[2, 3]);
        let slice12_out1 = unsqueeze25_out1.clone().slice(s![.., .., .., .., 2..]);
        let mul77_out1 = mul76_out1.mul(slice12_out1);
        let mul78_out1 = mul77_out1
            .mul((constant356_out1).unsqueeze_dims(&[0isize, 1isize, 2isize, 3isize]));
        let slice13_out1 = unsqueeze25_out1.slice(s![.., .., .., .., 0..2]);
        ...
        let reshape34_out1 = transpose31_out1.reshape([-1, 48, 18, 2]);
        ...
        let reshape35_out1 = transpose32_out1.reshape([-1, 1, 48, 18]);
        let concat28_out1 = burn::tensor::Tensor::cat(
            [gridsample4_out1, gridsample5_out1, gridsample6_out1].into(),
            3,
        );
        let mul80_out1 = concat28_out1.mul(reshape35_out1);
        let reducesum2_out1 = {
            mul80_out1.sum_dim(3usize).squeeze_dims::<3usize>(&[3])
        };
        let reshape36_out1 = reducesum2_out1.reshape(concat23_out1);
        ...
        let linear37_out1 = self.linear37.forward(concat29_out1);
...
impl Submodule7 {
    ...
    pub fn forward(
        &self,
        add64_out1: Tensor<3>,
        constant372_out1: Tensor<1>,
        constant360_out1: [i64; 1],
        constant347_out1: [i64; 1],
        constant395_out1: Tensor<1>,
        div9_out1: Tensor<2>,
        gather11_out1: Tensor<2>,
        div10_out1: Tensor<2>,
        gather14_out1: Tensor<2>,
        constant352_out1: Tensor<1>,
        add62_out1: Tensor<3>,
        constant351_out1: Tensor<1>,
        constant353_out1: Tensor<1>,
        ...
        let gather19_out1 = shape13_out1[0] as i64;
        ...
        let matmul47_out1 = softmax7_out1
            .matmul(constant372_out1.clone().unsqueeze_dims(&[-1isize]))
            .squeeze_dim::<1usize>(1usize);
        let unsqueeze26_out1 = [gather19_out1 as i64];
        let concat30_out1: [i64; 3usize] = [
            ...
            .try_into()
            .unwrap();
        let reshape38_out1 = matmul47_out1.reshape(concat30_out1);
        let gather20_out1 = {
            let sliced = reshape38_out1.clone().slice(s![.., .., 0]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let add65_out1 = (constant395_out1.clone())
            .unsqueeze_dims(&[0isize])
            .add(gather20_out1);
        let mul85_out1 = add65_out1.mul(div9_out1.clone());
        let sub17_out1 = gather11_out1.clone().sub(mul85_out1);
        let gather21_out1 = {
            let sliced = reshape38_out1.clone().slice(s![.., .., 1]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let add66_out1 = (constant395_out1.clone())
            .unsqueeze_dims(&[0isize])
            .add(gather21_out1);
        let mul86_out1 = add66_out1.mul(div10_out1.clone());
        let sub18_out1 = gather14_out1.clone().sub(mul86_out1);
        let gather22_out1 = {
            let sliced = reshape38_out1.clone().slice(s![.., .., 2]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let add67_out1 = (constant395_out1.clone())
            .unsqueeze_dims(&[0isize])
            .add(gather22_out1);
        let mul87_out1 = add67_out1.mul(div9_out1.clone());
        let add68_out1 = gather11_out1.clone().add(mul87_out1);
        let gather23_out1 = {
            let sliced = reshape38_out1.slice(s![.., .., 3]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let add69_out1 = (constant395_out1.clone())
            .unsqueeze_dims(&[0isize])
            .add(gather23_out1);
        let mul88_out1 = add69_out1.mul(div10_out1.clone());
        let add70_out1 = gather14_out1.clone().add(mul88_out1);
        let unsqueeze27_out1: Tensor<3> = sub17_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze28_out1: Tensor<3> = sub18_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze29_out1: Tensor<3> = add68_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze30_out1: Tensor<3> = add70_out1.unsqueeze_dims::<3>(&[-1]);
        let concat31_out1 = burn::tensor::Tensor::cat(
            [unsqueeze27_out1, unsqueeze28_out1, unsqueeze29_out1, unsqueeze30_out1]
                .into(),
            2,
        );
        let split_tensors = concat31_out1.split_with_sizes([1, 1, 1, 1].into(), 2);
        let [split9_out1, split9_out2, split9_out3, split9_out4] = split_tensors
            .try_into()
            .unwrap();
        let squeeze7_out1 = split9_out1.squeeze_dims::<2>(&[-1]);
        let squeeze8_out1 = split9_out2.squeeze_dims::<2>(&[-1]);
        let squeeze9_out1 = split9_out3.squeeze_dims::<2>(&[-1]);
        let squeeze10_out1 = split9_out4.squeeze_dims::<2>(&[-1]);
        let add71_out1 = squeeze7_out1.clone().add(squeeze9_out1.clone());
        let div16_out1 = add71_out1
            .div((constant352_out1.clone()).unsqueeze_dims(&[0isize]));
        let add72_out1 = squeeze8_out1.clone().add(squeeze10_out1.clone());
        let div17_out1 = add72_out1
            .div((constant352_out1.clone()).unsqueeze_dims(&[0isize]));
        let sub19_out1 = squeeze9_out1.sub(squeeze7_out1);
        let sub20_out1 = squeeze10_out1.sub(squeeze8_out1);
        let unsqueeze31_out1: Tensor<3> = div16_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze32_out1: Tensor<3> = div17_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze33_out1: Tensor<3> = sub19_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze34_out1: Tensor<3> = sub20_out1.unsqueeze_dims::<3>(&[-1]);
        let concat32_out1 = burn::tensor::Tensor::cat(
            [unsqueeze31_out1, unsqueeze32_out1, unsqueeze33_out1, unsqueeze34_out1]
                .into(),
            2,
        );
        let linear43_out1 = self.linear43.forward(concat32_out1.clone());
        ...
        let reshape39_out1 = linear45_out1.reshape([48, -1, 16]);
        let transpose36_out1 = reshape39_out1.permute([1, 0, 2]);
        let reshape40_out1 = linear46_out1.reshape([48, -1, 16]);
        let reshape41_out1 = linear47_out1.reshape([48, -1, 16]);
        ...
        let reshape43_out1 = linear48_out1.reshape([48, -1, 128]);
        ...
        let reshape44_out1 = linear49_out1.reshape([-1, 48, 8, 18, 2]);
        let linear50_out1 = self.linear50.forward(add77_out1);
        let reshape45_out1 = linear50_out1.reshape([-1, 48, 8, 18]);
        let softmax9_out1 = burn::tensor::activation::softmax(reshape45_out1, 3);
        let mul91_out1 = reshape44_out1
            .mul((constant300_out1).unsqueeze_dims(&[0isize, 1isize, 2isize]));
        let unsqueeze35_out1: Tensor<5> = concat32_out1.unsqueeze_dims::<5>(&[2, 3]);
        let slice16_out1 = unsqueeze35_out1.clone().slice(s![.., .., .., .., 2..]);
        let mul92_out1 = mul91_out1.mul(slice16_out1);
        let mul93_out1 = mul92_out1
            .mul(
                (constant356_out1.clone())
                    .unsqueeze_dims(&[0isize, 1isize, 2isize, 3isize]),
            );
        let slice17_out1 = unsqueeze35_out1.slice(s![.., .., .., .., 0..2]);
        ...
        let reshape46_out1 = transpose41_out1.reshape([-1, 48, 18, 2]);
        ...
        let reshape47_out1 = transpose42_out1.reshape([-1, 1, 48, 18]);
        let concat33_out1 = burn::tensor::Tensor::cat(
            [gridsample7_out1, gridsample8_out1, gridsample9_out1].into(),
            3,
        );
        let mul95_out1 = concat33_out1.mul(reshape47_out1);
        let reducesum3_out1 = {
            mul95_out1.sum_dim(3usize).squeeze_dims::<3usize>(&[3])
        };
        let reshape48_out1 = reducesum3_out1.reshape(concat23_out1);
        ...
        let linear51_out1 = self.linear51.forward(concat34_out1);
        ...
        let gather24_out1 = shape14_out1[0] as i64;
        ...
        let matmul63_out1 = softmax10_out1
            .matmul(constant372_out1.unsqueeze_dims(&[-1isize]))
            .squeeze_dim::<1usize>(1usize);
        let unsqueeze36_out1 = [gather24_out1 as i64];
        let concat35_out1: [i64; 3usize] = [
            ...
            .try_into()
            .unwrap();
        let reshape50_out1 = matmul63_out1.reshape(concat35_out1);
        let gather25_out1 = {
            let sliced = reshape50_out1.clone().slice(s![.., .., 0]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let add87_out1 = (constant395_out1.clone())
            .unsqueeze_dims(&[0isize])
            .add(gather25_out1);
        let mul100_out1 = add87_out1.mul(div9_out1.clone());
        let sub25_out1 = gather11_out1.clone().sub(mul100_out1);
        let gather26_out1 = {
            let sliced = reshape50_out1.clone().slice(s![.., .., 1]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let add88_out1 = (constant395_out1.clone())
            .unsqueeze_dims(&[0isize])
            .add(gather26_out1);
        let mul101_out1 = add88_out1.mul(div10_out1.clone());
        let sub26_out1 = gather14_out1.clone().sub(mul101_out1);
        let gather27_out1 = {
            let sliced = reshape50_out1.clone().slice(s![.., .., 2]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let add89_out1 = (constant395_out1.clone())
            .unsqueeze_dims(&[0isize])
            .add(gather27_out1);
        let mul102_out1 = add89_out1.mul(div9_out1);
        let add90_out1 = gather11_out1.add(mul102_out1);
        let gather28_out1 = {
            let sliced = reshape50_out1.slice(s![.., .., 3]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let add91_out1 = (constant395_out1).unsqueeze_dims(&[0isize]).add(gather28_out1);
        let mul103_out1 = add91_out1.mul(div10_out1);
        let add92_out1 = gather14_out1.add(mul103_out1);
        let unsqueeze37_out1: Tensor<3> = sub25_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze38_out1: Tensor<3> = sub26_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze39_out1: Tensor<3> = add90_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze40_out1: Tensor<3> = add92_out1.unsqueeze_dims::<3>(&[-1]);
        let concat36_out1 = burn::tensor::Tensor::cat(
            [unsqueeze37_out1, unsqueeze38_out1, unsqueeze39_out1, unsqueeze40_out1]
                .into(),
            2,
        );
        let split_tensors = concat36_out1.split_with_sizes([1, 1, 1, 1].into(), 2);
        let [split11_out1, split11_out2, split11_out3, split11_out4] = split_tensors
            .try_into()
            .unwrap();
        let squeeze11_out1 = split11_out1.squeeze_dims::<2>(&[-1]);
        let squeeze12_out1 = split11_out2.squeeze_dims::<2>(&[-1]);
        let squeeze13_out1 = split11_out3.squeeze_dims::<2>(&[-1]);
        let squeeze14_out1 = split11_out4.squeeze_dims::<2>(&[-1]);
        let add93_out1 = squeeze11_out1.clone().add(squeeze13_out1.clone());
        let div21_out1 = add93_out1
            .div((constant352_out1.clone()).unsqueeze_dims(&[0isize]));
        let add94_out1 = squeeze12_out1.clone().add(squeeze14_out1.clone());
        let div22_out1 = add94_out1.div((constant352_out1).unsqueeze_dims(&[0isize]));
        let sub27_out1 = squeeze13_out1.sub(squeeze11_out1);
        let sub28_out1 = squeeze14_out1.sub(squeeze12_out1);
        let unsqueeze41_out1: Tensor<3> = div21_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze42_out1: Tensor<3> = div22_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze43_out1: Tensor<3> = sub27_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze44_out1: Tensor<3> = sub28_out1.unsqueeze_dims::<3>(&[-1]);
        let concat37_out1 = burn::tensor::Tensor::cat(
            [unsqueeze41_out1, unsqueeze42_out1, unsqueeze43_out1, unsqueeze44_out1]
                .into(),
            2,
        );
        let linear57_out1 = self.linear57.forward(add84_out1);
        let reshape51_out1 = add86_out1.reshape([-1, 48, 4, 33]);
        ...
        let reshape52_out1 = concat38_out1.reshape(concat35_out1);
        ...
        let unsqueeze45_out1: Tensor<4> = concat37_out1.unsqueeze_dims::<4>(&[0]);
        let unsqueeze46_out1: Tensor<4> = add95_out1.unsqueeze_dims::<4>(&[0]);
        let gather29_out1 = {
            let sliced = unsqueeze46_out1.slice(s![- 1, .., .., ..]);
            sliced.squeeze_dim::<3usize>(0)
        };
        let gather30_out1 = {
            let sliced = unsqueeze45_out1.slice(s![- 1, .., .., ..]);
            sliced.squeeze_dim::<3usize>(0)
        };
        let split_tensors = gather30_out1.split_with_sizes([1, 1, 1, 1].into(), 2);
        let [split12_out1, split12_out2, split12_out3, split12_out4] = split_tensors
            .try_into()
            .unwrap();
        let squeeze15_out1 = split12_out1.squeeze_dims::<2>(&[-1]);
        let squeeze16_out1 = split12_out2.squeeze_dims::<2>(&[-1]);
        let squeeze17_out1 = split12_out3.squeeze_dims::<2>(&[-1]);
        let squeeze18_out1 = split12_out4.squeeze_dims::<2>(&[-1]);
        let mul104_out1 = squeeze17_out1
            .mul((constant356_out1.clone()).unsqueeze_dims(&[0isize]));
        let sub29_out1 = squeeze15_out1.clone().sub(mul104_out1.clone());
        let mul105_out1 = squeeze18_out1
            .mul((constant356_out1).unsqueeze_dims(&[0isize]));
        let sub30_out1 = squeeze16_out1.clone().sub(mul105_out1.clone());
        let add96_out1 = squeeze15_out1.add(mul104_out1);
        let add97_out1 = squeeze16_out1.add(mul105_out1);
        let unsqueeze47_out1: Tensor<3> = sub29_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze48_out1: Tensor<3> = sub30_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze49_out1: Tensor<3> = add96_out1.unsqueeze_dims::<3>(&[-1]);
        let unsqueeze50_out1: Tensor<3> = add97_out1.unsqueeze_dims::<3>(&[-1]);
        let concat39_out1 = burn::tensor::Tensor::cat(
            [unsqueeze47_out1, unsqueeze48_out1, unsqueeze49_out1, unsqueeze50_out1]
                .into(),
            2,
        );
        let tile4_out1 = orig_target_sizes.repeat(&[1, 2]);
        let unsqueeze51_out1: Tensor<3, Int> = tile4_out1.unsqueeze_dims::<3>(&[1]);
        let cast1_out1 = unsqueeze51_out1.float().cast(burn::tensor::DType::F32);
        let mul106_out1 = concat39_out1.mul(cast1_out1);
        (mul106_out1, gather29_out1)
    }
}
#[derive(Module, Debug)]
pub struct Submodule8 {
    constant403: burn::module::Param<Tensor<2, Int>>,
    constant396: burn::module::Param<Tensor<1, Int>>,
    constant385: burn::module::Param<Tensor<1, Int>>,
    constant376: burn::module::Param<Tensor<1, Int>>,
    constant377: burn::module::Param<Tensor<1, Int>>,
    constant398: burn::module::Param<Tensor<1, Int>>,
    constant362: burn::module::Param<Tensor<1, Int>>,
...
impl Submodule8 {
    #[allow(unused_variables)]
    pub fn new(device: &Device) -> Self {
        let constant403: burn::module::Param<Tensor<2, Int>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                2,
                Int,
            >::zeros([48, 1], (device, burn::tensor::DType::I64)),
            device.clone(),
            false,
            [48, 1].into(),
        );
        let constant396: burn::module::Param<Tensor<1, Int>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                1,
                Int,
        ...
        let constant385: burn::module::Param<Tensor<1, Int>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                1,
                Int,
        ...
        let constant376: burn::module::Param<Tensor<1, Int>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                1,
                Int,
        ...
        let constant377: burn::module::Param<Tensor<1, Int>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                1,
                Int,
        ...
        let constant362: burn::module::Param<Tensor<1, Int>> = burn::module::Param::uninitialized(
            burn::module::ParamId::new(),
            move |device, _require_grad| Tensor::<
                1,
                Int,
    ...
    pub fn forward(
        &self,
        mul106_out1: Tensor<3>,
        constant349_out1: [i64; 1],
        gather29_out1: Tensor<3>,
        constant347_out1: [i64; 1],
    ) -> (Tensor<2, Int>, Tensor<3>, Tensor<2>, Tensor<2>, Tensor<2, Int>) {
        let gather31_out1 = {
            let sliced = mul106_out1.clone().slice(s![.., .., 1]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let shape15_out1: [i64; 2] = {
            let axes = &gather31_out1.dims()[0..2];
            let mut output = [0i64; 2];
            for i in 0..2 {
                output[i] = axes[i] as i64;
            }
        ...
        let expand1_out1 = {
            ...
            let input_dims = constantofshape3_out1.dims();
            ...
            constantofshape3_out1.expand(shape)
        };
        let shape16_out1: [i64; 3] = {
            let axes = &mul106_out1.clone().dims()[0..3];
            let mut output = [0i64; 3];
            for i in 0..3 {
                output[i] = axes[i] as i64;
            }
        ...
        let reshape53_out1 = range1_out1.reshape([-1, 1, 1]);
        let constant403_out1 = self.constant403.val();
        let add98_out1 = reshape53_out1
            .clone()
            .add((constant403_out1.clone()).unsqueeze_dims(&[0isize]));
        let add99_out1 = add98_out1
            .add(
                Tensor::<
                    1,
                    burn::tensor::Int,
                >::from_data(
                    burn::tensor::TensorData::from(&constant349_out1 as &[i64]),
                    &self.device,
                ),
            );
        let shape17_out1: [i64; 3] = {
            let axes = &add99_out1.dims()[0..3];
            let mut output = [0i64; 3];
            for i in 0..3 {
                output[i] = axes[i] as i64;
            }
        ...
        let equal1_out1 = {
            ...
            shape_tensor.equal(constant396_out1.clone())
        };
        ...
        >::from_data(
            ...
            .mask_where(equal1_out1, constant385_out1.clone());
        let expand2_out1 = {
        ...
        let unsqueeze52_out1: Tensor<4, Int> = expand2_out1.unsqueeze_dims::<4>(&[-1]);
        let expand3_out1 = {
            let onnx_shape: [i64; 3usize] = TryInto::<
                [i64; 3usize],
            >::try_into(where1_out1.to_data().convert::<i64>().as_slice().unwrap())
                .unwrap();
            let input_dims = constant403_out1.clone().dims();
            let mut shape = onnx_shape;
            #[allow(clippy::needless_range_loop)]
            for i in 0..2usize {
            ...
            constant403_out1.clone().expand(shape)
        };
        let unsqueeze53_out1: Tensor<4, Int> = expand3_out1.unsqueeze_dims::<4>(&[-1]);
        let expand4_out1 = {
            let onnx_shape: [i64; 3usize] = TryInto::<
                [i64; 3usize],
            >::try_into(where1_out1.to_data().convert::<i64>().as_slice().unwrap())
            ...
            let input_tensor = Tensor::<
                ...
                Int,
            >::from_data(
                burn::tensor::TensorData::from(constant349_out1.as_slice()),
                &self.device,
            );
            let input_dims = [1];
            let mut shape = onnx_shape;
        ...
        let unsqueeze54_out1: Tensor<4, Int> = expand4_out1.unsqueeze_dims::<4>(&[-1]);
        let concat40_out1 = burn::tensor::Tensor::cat(
            [unsqueeze52_out1, unsqueeze53_out1, unsqueeze54_out1].into(),
            3,
        );
        let reshape54_out1 = expand1_out1.reshape(shape17_out1);
        let scatternd1_out1 = {
            let __nd_data_dims = mul106_out1.dims();
            let __nd_indices = concat40_out1.cast(burn::tensor::DType::I64);
        ...
        let gather33_out1 = {
            let sliced = scatternd1_out1.clone().slice(s![.., .., 3]);
            sliced.squeeze_dim::<2usize>(2)
        };
        let shape18_out1: [i64; 2] = {
            let axes = &gather33_out1.dims()[0..2];
            let mut output = [0i64; 2];
            for i in 0..2 {
                output[i] = axes[i] as i64;
            }
        ...
        >::from_data(
            ...
            .expand(shape18_out1);
        let expand5_out1 = {
            let onnx_shape: [i64; 2usize] = shape18_out1;
            let input_dims = constantofshape4_out1.dims();
            let mut shape = onnx_shape;
            #[allow(clippy::needless_range_loop)]
            for i in 0..2usize {
                let dim_offset = 2usize - 2usize + i;
            ...
            constantofshape4_out1.expand(shape)
        };
        ...
        let gather34_out1 = shape19_out1[0] as i64;
        let range2_out1 = {
            let __start = 0i64;
            let __limit = gather34_out1;
            let __delta = 1i64;
            assert!(__delta != 0);
            let __n = ((__limit - __start) as f64 / __delta as f64).ceil().max(0.0)
                as i64;
        ...
        let reshape55_out1 = range2_out1.reshape([-1, 1, 1]);
        let add100_out1 = reshape55_out1
            .clone()
            .add((constant403_out1.clone()).unsqueeze_dims(&[0isize]));
        let constant376_out1 = self.constant376.val();
        let add101_out1 = add100_out1
            .add((constant376_out1.clone()).unsqueeze_dims(&[0isize, 1isize]));
        let shape20_out1: [i64; 3] = {
            let axes = &add101_out1.dims()[0..3];
            let mut output = [0i64; 3];
            for i in 0..3 {
                output[i] = axes[i] as i64;
            }
        ...
        let equal2_out1 = {
            let shape_tensor = Tensor::<
                ...
                Int,
            >::from_data(
                burn::tensor::TensorData::from(shape20_out1.as_slice()),
                &self.device,
            );
            shape_tensor.equal(constant396_out1)
        };
        let where2_out1 = Tensor::<
            1,
            burn::tensor::Int,
        >::from_data(
                burn::tensor::TensorData::from(&shape20_out1 as &[i64]),
                &self.device,
            )
            .mask_where(equal2_out1, constant385_out1);
        let expand6_out1 = {
            let onnx_shape: [i64; 3usize] = TryInto::<
                [i64; 3usize],
            >::try_into(where2_out1.to_data().convert::<i64>().as_slice().unwrap())
                .unwrap();
            let input_dims = reshape55_out1.dims();
            let mut shape = onnx_shape;
            #[allow(clippy::needless_range_loop)]
            for i in 0..3usize {
                let dim_offset = 3usize - 3usize + i;
            ...
            reshape55_out1.expand(shape)
        };
        let unsqueeze55_out1: Tensor<4, Int> = expand6_out1.unsqueeze_dims::<4>(&[-1]);
        let expand7_out1 = {
            let onnx_shape: [i64; 3usize] = TryInto::<
                [i64; 3usize],
            >::try_into(where2_out1.to_data().convert::<i64>().as_slice().unwrap())
                .unwrap();
            let input_dims = constant403_out1.dims();
            let mut shape = onnx_shape;
            #[allow(clippy::needless_range_loop)]
            for i in 0..2usize {
                let dim_offset = 3usize - 2usize + i;
            ...
            constant403_out1.expand(shape)
        };
        let unsqueeze56_out1: Tensor<4, Int> = expand7_out1.unsqueeze_dims::<4>(&[-1]);
        let expand8_out1 = {
            let onnx_shape: [i64; 3usize] = TryInto::<
                [i64; 3usize],
            >::try_into(where2_out1.to_data().convert::<i64>().as_slice().unwrap())
                .unwrap();
            let input_dims = constant376_out1.dims();
            let mut shape = onnx_shape;
            #[allow(clippy::needless_range_loop)]
            for i in 0..1usize {
                let dim_offset = 3usize - 1usize + i;
            ...
            constant376_out1.expand(shape)
        };
        let unsqueeze57_out1: Tensor<4, Int> = expand8_out1.unsqueeze_dims::<4>(&[-1]);
        let concat41_out1 = burn::tensor::Tensor::cat(
            [unsqueeze55_out1, unsqueeze56_out1, unsqueeze57_out1].into(),
            3,
        );
        let reshape56_out1 = expand5_out1.reshape(shape20_out1);
        let scatternd2_out1 = {
            let __nd_data_dims = scatternd1_out1.dims();
            let __nd_indices = concat41_out1.cast(burn::tensor::DType::I64);
            let __nd_idx_dims = __nd_indices.dims();
            let __nd_k = __nd_idx_dims[4 - 1];
            let mut __nd_dim_sizes: alloc::vec::Vec<i64> = alloc::vec::Vec::with_capacity(
                __nd_k,
        ...
        let sigmoid54_out1 = burn::tensor::activation::sigmoid(gather29_out1);
        let flatten1_out1 = {
        ...
        let mod1_out1 = {
            ...
            let __rhs = (constant377_out1.clone()).unsqueeze_dims(&[0isize]);
        ...
        let div23_out1 = topk3_out2
            .clone()
            .div((constant377_out1).unsqueeze_dims(&[0isize]));
        let unsqueeze58_out1: Tensor<3, Int> = div23_out1.unsqueeze_dims::<3>(&[-1]);
        let expand9_out1 = {
            let onnx_shape: [i64; 3usize] = [1, 1, 4];
            let input_dims = unsqueeze58_out1.dims();
            let mut shape = onnx_shape;
            #[allow(clippy::needless_range_loop)]
            for i in 0..3usize {
                let dim_offset = 3usize - 3usize + i;
        ...
        let gather35_out1 = shape21_out1[0] as i64;
        let unsqueeze59_out1 = [gather35_out1 as i64];
        let concat42_out1: [i64; 2usize] = [&unsqueeze59_out1[..], &constant347_out1[..]]
            .concat()
            .try_into()
            .unwrap();
        ...
        let equal3_out1 = {
            let shape_tensor = Tensor::<
                ...
                Int,
            >::from_data(
                burn::tensor::TensorData::from(concat42_out1.as_slice()),
                &self.device,
            );
            shape_tensor.equal(constant398_out1)
        };
        ...
        let where3_out1 = Tensor::<
            ...
            burn::tensor::Int,
        >::from_data(
                burn::tensor::TensorData::from(&concat42_out1 as &[i64]),
                &self.device,
            )
            .mask_where(equal3_out1, constant362_out1);
...
impl Default for Model {
    fn default() -> Self {
        Self::from_file(
            "burn_models/rec_horizontal/meiki.text.rec.v0.960x32.bpk",
            &Default::default(),
        )
    }
}
...
impl Model {
    ...
    pub fn forward(
        ...
        let (
            ...
            .submodule3
            .forward(sub1_out1, conv2d47_out1, conv2d46_out1, constant347_out1.clone());
        let (
            gatherelements2_out1,
            clip1_out1,
            sigmoid49_out1,
            ...
            .forward(
                concat14_out1,
                constant352_out1.clone(),
                constant353_out1.clone(),
                constant347_out1.clone(),
            );
        let (add36_out1, constant360_out1, constant300_out1, concat23_out1) = self
            .submodule5
            .forward(
                gatherelements2_out1,
                clip1_out1,
        ...
        let (
            ...
            .forward(
                ...
                sigmoid49_out1,
                constant355_out1.clone(),
                constant360_out1.clone(),
                constant347_out1.clone(),
                constant351_out1.clone(),
                constant300_out1.clone(),
                constant356_out1.clone(),
                slice5_out1.clone(),
                slice6_out1.clone(),
                slice7_out1.clone(),
        ...
        let (mul106_out1, gather29_out1) = self
            .submodule7
            .forward(
                add64_out1,
                constant372_out1,
        ...
        let (
            ...
            .submodule8
            .forward(mul106_out1, constant349_out1, gather29_out1, constant347_out1);
        (
            gatherelements4_out1,
            gatherelements3_out1,
            topk3_out1,
