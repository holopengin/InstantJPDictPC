...
//! (#43), extended to the three zips this build ships in
//! `assets/dictionaries/`: Jitendex, KANJIDIC (English) and Kanjium pitch
//! accents.
//!
//! The install runs once per start on a worker thread, before any GUI work,
//! and is idempotent: a bundled zip is imported only when its stable title
//! family is absent from the database, and a completed install is marked
//! `built_in` so later starts skip it after one cheap query. Nothing is
//! downloaded — the zips travel next to the binary.
...
pub struct BundledDictionary {
    ...
    /// against, exactly as the catalog does: the zip's declared title may
    /// carry a revision suffix (`Jitendex.org [2026-08-11]`, `KANJIDIC
    /// [2026-258]`), the family is the part before it.
...
/// The zips this build ships, pinned in `assets/dictionaries/PROVENANCE.txt`.
pub static BUNDLED_DICTIONARIES: &[BundledDictionary] = &[
    BundledDictionary {
        file: "jitendex-yomitan.zip",
        family: "Jitendex.org",
        id: "jitendex",
        source: "stephenmk/stephenmk.github.io @ 2026.08.11.0",
    },
    BundledDictionary {
...
/// file picker, and later the catalog) — the bundled zip must never stack a
...
fn dictionary_is_present(db: &DictionaryDatabase, spec: &BundledDictionary) -> Result<bool> {
    let rows = db.dictionaries_by_family(spec.family)?;
...
/// import of ~40 MB never delays the GUI or the bootstrap thread. Lookups read
/// the database live, so no completion message is needed; failures are logged
...
mod tests {
    ...
    /// together they pin every match form the catalog describes.
