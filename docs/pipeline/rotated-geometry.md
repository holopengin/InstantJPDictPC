...
- The stored frame angle is the frame's **local-x-axis angle** from +x
  ...
  `RotatedBox.angle` (`src/models.rs:62`, `RotatedBox::new`).
...
- Verticality is the 1.25 aspect rule (`VERTICAL_MIN_ASPECT`,
  `src/models.rs:30`; `is_vertical`, `:115`). Near-square stays horizontal.
...
- `fit_quad` fits the boundary-walk points (`src/models.rs:278`); degenerate
  point sets (single point, pair, collinear) return `None`.
- `unclip` expands along the box's own axes keeping the centre
  (`src/models.rs:140`); `map_local_rect` maps upright local crops back
...
- Detection runs the real chain in pipeline order — `fit_components` →
  ...
  (`PPOCR_DET_THRESH`, `PPOCR_DET_UNCLIP_RATIO`, `src/ocr_engine.rs:21-23`,
...
- PC unit (`src/models.rs` tests): `rotated_horizontal_rect_fits_the_rect_it_was_made_from`,
...
- Conformance: `geometry-01-angle-conventions` (angle/vertical/axis-aligned
  ...
  `detection_cases` (`src/conformance.rs:288` / `:237`).
